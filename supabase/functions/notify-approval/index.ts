...
// Fires when staff approve a booking request. Confirms the dates directly with
// the renter. See notify-booking for the staff-facing "new request" notification.
Deno.serve(async (req) => {
  ...
  if (rental.status !== 'approved') {
    return new Response(JSON.stringify({ error: 'Rental is not approved' }), {
      status: 409,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
  ...
  const vehicleName = rental.inventory?.name || 'your rental';

  const html = renderEmail(
    "You're confirmed!",
    `
      <p>Hi ${escapeHtml(rental.renter_name || 'there')},</p>
      <p>Your booking with Lake Area Rentals is confirmed:</p>
      <p>
        <strong>Vehicle:</strong> ${escapeHtml(vehicleName)}<br>
        <strong>Pickup:</strong> ${escapeHtml(formatDate(rental.start_date))}<br>
        <strong>Return:</strong> ${escapeHtml(formatDate(rental.end_date))}
      </p>
      <p>If you have any questions before pickup, call us at (501) 250-6398 or reply to this email.</p>
      <p>See you soon!<br>Lake Area Rentals</p>
    `
  );
  ...
  const result = await sendEmail({
    ...
    subject: `You're confirmed — ${vehicleName}`,
    html,
  });
